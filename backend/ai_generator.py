...
"""
    ...
    def _process_rounds_recursive(self, context: RoundContext, tool_manager, max_rounds: int = 2) -> str:
        """
        ...
        tool_execution_result = self._execute_tools_for_round(response, context, tool_manager)
    ...
    def _execute_tools_for_round(self, response, context: RoundContext, tool_manager) -> ToolExecutionResult:
        ...
        for content_block in response.content:
            if content_block.type == "tool_use":
                try:
                    ...
                    context.errors.append(f"Round {context.current_round}: {result.error_message}")
                    break
        
        return result
    
    def _update_context_with_tool_results(self, context: RoundContext, response, tool_result: ToolExecutionResult):
        ...
        if tool_result.tool_results:
            context.messages.append({"role": "user", "content": tool_result.tool_results})
    ...
    def _handle_tool_execution_failure(self, tool_result: ToolExecutionResult, context: RoundContext) -> str:
